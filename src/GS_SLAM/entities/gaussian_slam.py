...
from src.GS_SLAM.entities.datasets import get_dataset
from src.GS_SLAM.entities.gaussian_model import GaussianModel
from src.GS_SLAM.entities.mapper import Mapper
from src.GS_SLAM.entities.tracker import Tracker
from src.GS_SLAM.entities.logger import Logger
from src.GS_SLAM.utils.io_utils import save_dict_to_ckpt, save_dict_to_yaml
from src.GS_SLAM.utils.mapper_utils import exceeds_motion_thresholds
from src.GS_SLAM.utils.utils import np2torch, setup_seed, torch2np
...
class GaussianSLAM(object):
    ...
    def __init__(self, config: dict, dataset) -> None:
        ...
        # return dataset: index, masked_color_data, pose, K, lidar
        ...
        # self.estimated_c2ws[0] = torch.from_numpy(self.dataset[0][4]

        #  try gt pose 
        self.estimated_c2ws = [dataset.get_transform(index) for index in range(len(dataset))]
