...
def draw_camera(K, R, t, w, h, scale=1, color=[0.8, 0.2, 0.8], img_path=None):
    """Create axis, plane and pyramid geometries in o3d format.
    :param K: calibration matrix (camera intrinsics)
    :param R: rotation matrix
    :param t: translation
    :param w: image width
    ...
    points_pixel = [
        ...
        [w, h, 1],
        [0, h, 1],
        [0, 0, 0]  # Camera center in homogeneous coordinates
    ]
    ...
    # Normalize the vertex positions by the maximum dimension to maintain aspect ratio
    vertices = np.array([
        [points[0][0], points[0][1], 0],  # Bottom-left
        [points[1][0], points[1][1], 0],  # Bottom-right
        [points[2][0], points[2][1], 0],  # Top-right
        [points[3][0], points[3][1], 0]   # Top-left
    ])
    ...
    # Set UV coordinates, Open3D's UV origin is at the bottom-left
    v_uv = np.array([
        [0, 1],  # Bottom-left
        [1, 1],  # Bottom-right
        [1, 0],  # Top-right
    ...
    # Translate mesh so that its center is at the camera center
    mesh.translate([0, 0, scale])
    mesh.transform(T)
